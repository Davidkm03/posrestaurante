    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">Agrega un nuevo producto al catálogo</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <x-card title="Información Básica">
                    <div class="space-y-4">
                        <x-input name="name" label="Nombre del producto" required placeholder="Ej: Hamburguesa Clásica" />

                        <div class="grid grid-cols-2 gap-4">
                            <x-select name="category_id" label="Categoría" required :options="$categories->pluck('name', 'id')->toArray()" />
                            <x-input name="sku" label="SKU" placeholder="Código único" />
                        </div>

                        <x-textarea name="description" label="Descripción" rows="3" placeholder="Descripción del producto..." />
                    </div>
                </x-card>
                ...
                <x-card title="Precios e Impuestos">
                    <div class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <x-input type="number" name="price" label="Precio de venta" required prefix="$" step="100" min="0" />
                            <x-input type="number" name="cost" label="Costo" prefix="$" step="100" min="0" />
                        </div>

                        <div class="grid grid-cols-3 gap-4">
                            <x-select name="tax_type" label="Tipo de impuesto" required :options="['01' => 'IVA', '04' => 'Impoconsumo', 'ZZ' => 'Exento']" />
                            <x-input type="number" name="tax_percentage" label="% Impuesto" required suffix="%" value="19" min="0" max="100" />
                            <div class="flex items-end pb-2">
                                <label class="flex items-center gap-2">
                                    <input type="checkbox" name="tax_included" value="1" checked class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                ...
                <x-card title="Modificadores">
                    <p class="text-sm text-gray-500 mb-4">Selecciona los grupos de modificadores disponibles para este producto</p>

                    @if($modifierGroups->isEmpty())
                        <p class="text-sm text-gray-400">No hay grupos de modificadores configurados</p>
                    @else
                        <div class="grid grid-cols-2 gap-3">
                            @foreach($modifierGroups as $group)
                                <label class="flex items-center gap-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
                                    <input type="checkbox" name="modifier_groups[]" value="{{ $group->id }}" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                    <div>
                                        <p class="font-medium text-gray-900">{{ $group->name }}</p>
                                        <p class="text-xs text-gray-500">{{ $group->modifiers_count ?? 0 }} opciones</p>
                                    </div>
                                </label>
                            @endforeach
                        </div>
                ...
                <x-card title="Configuración">
                    <div class="space-y-4">
                        ...
                        <label class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            ...
                            <input type="checkbox" name="show_in_pos" value="1" checked class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        </label>

                        <label class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            <span class="text-sm font-medium text-gray-700">Permitir notas</span>
                            <input type="checkbox" name="allow_notes" value="1" checked class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        </label>
                    </div>
                </x-card>
