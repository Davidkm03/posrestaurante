    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Nota de Débito {{ $debitNote->number ?? '#' . $debitNote->id }}
            </h2>
        </div>
    </x-slot>
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                <div class="p-6 border-b bg-gradient-to-r from-green-50 to-emerald-50">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h3 class="text-xl font-bold text-gray-900">{{ $debitNote->number ?? 'ND-' . str_pad($debitNote->id, 6, '0', STR_PAD_LEFT) }}</h3>
                                <p class="text-sm text-gray-500">{{ $debitNote->created_at->format('d/m/Y H:i') }}</p>
                            </div>
                        </div>
                        <span class="px-4 py-2 rounded-full text-sm font-medium
                            {{ $debitNote->status === 'approved' ? 'bg-green-100 text-green-800' : '' }}
                            {{ $debitNote->status === 'pending' ? 'bg-yellow-100 text-yellow-800' : '' }}
                            {{ $debitNote->status === 'rejected' ? 'bg-red-100 text-red-800' : '' }}">
                            {{ $debitNote->status === 'approved' ? '✓ Aprobada' : ($debitNote->status === 'pending' ? '⏳ Pendiente' : '✗ Rechazada') }}
                        </span>
                    </div>
                </div>
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div class="p-4 bg-gray-50 rounded-lg">
                            ...
                            @if($debitNote->invoice)
                                <a href="{{ route('admin.invoices.show', $debitNote->invoice_id) }}" 
                                   class="text-blue-600 hover:underline font-medium">
                                    {{ $debitNote->invoice->invoice_number }}
                                </a>
                                <p class="text-sm text-gray-500 mt-1">
                                    Total original: ${{ number_format($debitNote->invoice->total, 0) }}
                                </p>
                            @else
                        ...
                        <div class="p-4 bg-gray-50 rounded-lg">
                            ...
                            @if($debitNote->customer)
                                ...
                                <p class="text-sm text-gray-500">
                                    {{ $debitNote->customer->document_type }}: {{ $debitNote->customer->document_number }}
                                </p>
                            @else
                    ...
                    <div class="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                        <h4 class="text-sm font-medium text-yellow-800 mb-1">Motivo del Cargo</h4>
                        <p class="text-gray-700">{{ $debitNote->reason }}</p>
                    </div>

                    <!-- Items -->
                    <div>
                        <h4 class="font-medium text-gray-900 mb-3">Conceptos</h4>
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @forelse($debitNote->items ?? [] as $item)
                                        <tr>
                                            ...
                                            <td class="px-4 py-3 text-right font-medium text-gray-900">${{ number_format($item->quantity * $item->unit_price, 0) }}</td>
                                        </tr>
                                    @empty
                                        <tr>
                    ...
                    <div class="flex justify-end">
                        <div class="w-72 space-y-2">
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-medium">${{ number_format($debitNote->subtotal, 0) }}</span>
                            </div>
                            @if($debitNote->tax > 0)
                                <div class="flex justify-between text-sm">
                                    <span class="text-gray-500">IVA:</span>
                                    <span class="font-medium">${{ number_format($debitNote->tax, 0) }}</span>
                                </div>
                            @endif
                            <div class="flex justify-between text-lg font-bold border-t pt-2">
                                <span class="text-gray-900">Total Cargo:</span>
                                <span class="text-green-600">+${{ number_format($debitNote->total, 0) }}</span>
                            </div>
                        </div>
                    </div>

                    <!-- DIAN Info -->
                    @if($debitNote->dian_uuid)
                        <div class="p-4 bg-green-50 rounded-lg border border-green-200">
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    ...
                                    <p class="font-mono text-xs text-gray-700 break-all">{{ $debitNote->dian_uuid }}</p>
                                </div>
                                <div>
                                    <span class="text-gray-500">Validado:</span>
                                    <p class="text-gray-700">{{ $debitNote->dian_validated_at?->format('d/m/Y H:i') ?? 'Pendiente' }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
